...
new TimelineMax({onStart:populate})
		...
		.set('.foundTxt',	{left:0, top:30, width:1024, fontSize:35, textAlign:'center', textContent:'Знайдіть відповідну пару...', fontWeight:300, letterSpacing:0.25})
...
function populate() {
    ...
    matched = false;
		msTilHint = 5000;
		TweenMax.set(hintBtn, {autoAlpha:0, textContent:"Підказка?", fontSize:25});
    ...
    if (matched) return; //prevent user from repeatedly clicking the matching pair
        ...
        matched = true;
				...
				new TimelineMax({onComplete:populate})
					.set('.foundTxt', {textContent:'Знайдено: '+found, fontWeight:500}, 0)
...
function updateTime(){
		if (timeLeft>0){
			...
			timeInt = undefined;
			if (found==0) TweenMax.set('.foundTxt', {textContent:'Знайдено: 0', fontWeight:500});
			TweenMax.set('.endTxt', {textContent:hint+" Спробуйте ще раз? "+hint, fontSize:35});
...
replayBtn.onclick = replayBtn.ontouchend = function(){
  new TimelineMax()			
			...
			.call(function(){ // Reset game...
				TweenMax.set('.foundTxt', {textContent:'Знайдено: 0'});
				TweenMax.set('.timeTxt', {textContent:'60:00'});
				found = 0;
